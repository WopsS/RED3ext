...
void ShowLastErrorMessage(uint32_t aType, const std::wstring_view aAdditionalText = "", Args&&... aArgs)
{
    ...
    auto caption = fmt::format(L"RED3ext (error {})", error);
...
template<typename Char>
struct fmt::formatter<FileVer, Char> : formatter<basic_string_view<Char>, Char>
{
    template<typename FormatContext>
    auto format(const FileVer& aFileVersion, FormatContext& ctx) const
    {
