...
const std::string_view Config::GetVersion() const
{
    return m_version;
}
...
void Config::Load(const std::filesystem::path& aFile)
{
    try
    {
        ...
        {
            auto version = toml::find<std::string>(config, "version");
            if (version == "0.0")
            {
                LoadV0(config);
            }
            else
            {
                SHOW_MESSAGE_BOX_AND_EXIT_FILE_LINE(
                    ...
                    Utils::Widen(version), Utils::Widen(MinSupportedVersion), Utils::Widen(MaxSupportedVersion), aFile);
            }
        }
        else
        {
